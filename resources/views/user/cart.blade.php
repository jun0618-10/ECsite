                     ...
                     <div class="md:flex md:items-center mb-2 ">
                        <div class="md:w-3/12">
                            @if($product->imageFirst->filename !== null)
                                    <img class="w-20 h-20 md:w-32 md:h-32 object-cover"  src="{{ asset('storage/products/' .$product->imageFirst->filename )}}">
                                    @else
                                    <img src="">
                                    @endif
                        </div>
                        ...
                        <div class="md:w-3/12 flex justify-around">
                            ...
                            <div>{{ number_format($product->pivot->quantity * $product->price )}}<span>円(税込)</span></div>
                        </div>
                    ...
                    @if(count($products) > 0)
                        ...
                        <div class="flex items-center mb-4 space-x-4">
                            ...
                            <div class="w-3/12 flex justify-between">
                                <div>{{ $product->pivot->quantity }}個</div>
                                <div>{{ number_format($product->pivot->quantity * $product->price) }}<span class="ml-1">円(税込)</span></div>
                            </div>
                        ...
                        @endforeach
                        <div class="my-2">
                            小計: {{ number_format($total_price)}}<span class="text-sm

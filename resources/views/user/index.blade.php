                ...
                <form method="get" action="{{ route('user.items.index')}}">
                    ...
                    <div class="flex">
                        <div>
                            ...
                            @if(\Request::get('sort') === Common::SORT_ORDER['recommend'] )
                                selected
                                @endif>おすすめ順
                                </option>

                            <option value="{{ Common::SORT_ORDER['higherPrice']}}"
                            @if(\Request::get('sort') === Common::SORT_ORDER['higherPrice'] )
                                selected
                                @endif>料金の高い順
                                </option>

                            <option value="{{ Common::SORT_ORDER['lowerPrice']}}"
                            @if(\Request::get('sort') === Common::SORT_ORDER['lowerPrice'] )
                                selected
                                @endif>料金の安い
                                </option>

                            <option value="{{ Common::SORT_ORDER['later']}}"
                            @if(\Request::get('sort') === Common::SORT_ORDER['later'] )
                                selected
                                @endif>新しい順
                                </option>

                            <option value="{{ Common::SORT_ORDER['older']}}"
                            @if(\Request::get('sort') === Common::SORT_ORDER['older'] )
                                selected
                                @endif>古い順番
                                </option>
                        ...
                        <a href="{{ route('user.items.show',['item' => $product->id]) }}">
                            <div class="border rounded-md p-2 md:p-4">
                                ...
                                <x-thumbnail filename="{{$product->filename ?? ''}}" type="products" />
                                    <div class="mt-4">
                                        <h3 class="text-gray-500 text-xs tracking-widest title-font mb-1">{{ $product->category ?? 'カテゴリなし'}}</h3>

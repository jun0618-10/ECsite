            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="post" action="{{ route('owner.products.update',['product' => $product->id]) }}">
                        ...
                        <x-select-image :images="$images" name="image5" currentId="{{$product->image5}}"currentImage="{{$product->imageFirst->filename ??''}}"/>
                        <div class="p-2 w-1/2 mx-auto">
                            <div class="relative flex justify-around">
                    ...
                    <form id="delete_{{$product->id}}" method="post" action="{{ route('owner.products.destroy', ['product' => $product->id]) }}" style="display:inline;">
                        @csrf
                        @method('delete')
                        <div class="p-2 w-full flex justify-around mt-32">
                        <a href="#" data-id="{{ $product->id }}" onclick="deletePost(this)" class="text-white bg-red-400 border-0 py-2 px-4 focus:outline-none hover:bg-red-500 rounded text-lg">削除</a>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
 ...
 if (confirm('本当に削除してもいいですか？')) {
     var form = document.getElementById('delete_' + element.dataset.id);
     if (form) {
         form.submit();
     } else {
         console.error('フォームが見つかりません: delete_' + element.dataset.id);
     }
 }
}
 </script>
